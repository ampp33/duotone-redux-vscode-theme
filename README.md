...
Every theme is generated from `src/buildTheme.js`, a single shared formula driven by two small starter palettes per variant (`src/themes.js`): a cool/neutral "UNO" tint scale and a warm "DUO" accent scale. Each variant also gets a "(Higher Contrast)" counterpart, generated by the same formula with its darkest text tiers swapped to white. Run `npm run build` after editing either file to regenerate all `themes/*.json` outputs.
